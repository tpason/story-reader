...
function isDefaultHomeCatalog(params: HomeSearchParams, queryText?: string, authorText?: string) {
  return (
    !queryText &&
    !authorText &&
    params.hot !== "true" &&
...
async function loadHomeStories(params: HomeSearchParams, queryText?: string, authorText?: string) {
  if (isDefaultHomeCatalog(params, queryText, authorText)) {
    return getCachedDefaultHomeStories();
  }
  return listStoriesCursor({
    limit: 24,
    queryText,
    author: authorText,
    hot: params.hot === "true",
    ...
    sort:
      params.sort === "chapters" ||
      params.sort === "hot" ||
      params.sort === "title" ||
      params.sort === "updated" ||
      params.sort === "trending" ||
      params.sort === "reader_rank"
        ? params.sort
        : undefined
  });
}
...
async function HomeCategoryPortal({
  params,
  filterQueryBase,
}: {
  params: HomeSearchParams;
  filterQueryBase: Record<string, string | undefined>;
}) {
  const categories = await getCachedCategories(12);
...
async function SearchActiveChrome({
  params,
  queryText,
  authorText,
}: {
  ...
  const stories = await loadHomeStories(params, queryText, authorText);
...
async function HomeLibraryStickyPanel({
  params,
  queryText,
  authorText,
  isSearchActive,
  ...
  const [stories, categories] = await Promise.all([
    loadHomeStories(params, queryText, authorText),
...
function HomeLibraryFallback() {
  return (
    <div className="library-sticky-panel" aria-busy="true" aria-label="Đang tải thư viện">
...
export default async function Home({ searchParams }: HomeProps) {
  ...
  // Sync-only — do not await catalog here so discovery Suspense can stream first.
      ...
      <div className="page-wrap" data-search-active={isSearchActive ? "true" : undefined}>
        ...
        */}
        ...
        {isSearchActive ? (
          <Suspense
            fallback={
              <div className="search-active-header" aria-busy="true">
                <div className="xi-skel" style={{ height: 18, width: 160 }} />
              </div>
            }
          >
            <SearchActiveChrome params={params} queryText={queryText} authorText={authorText} />
          </Suspense>
        ) : (
          <div className="home-story-flow home-story-flow--dense">
            ...
            <Suspense fallback={null}>
              <HomeCategoryPortal params={params} filterQueryBase={filterQueryBase} />
            </Suspense>
        ...
        <Suspense fallback={<HomeLibraryFallback />}>
          <HomeLibraryStickyPanel
            params={params}
            queryText={queryText}
            authorText={authorText}
            isSearchActive={isSearchActive}
            libraryKey={libraryKey}
            filterQueryBase={filterQueryBase}
          />
        </Suspense>
      </div>
    </main>
