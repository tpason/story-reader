...
export function ThreeReaderProgress({ progress }: ThreeReaderProgressProps) {
  ...
  useEffect(() => {
    ...
    const parchment = new THREE.Mesh(
      new THREE.BoxGeometry(5.6, 0.052, 0.035),
      new THREE.MeshBasicMaterial({ color: "#f5d75e", transparent: true, opacity: 0.22 })
    );

    // Gold linh-qi fill
    const fill = new THREE.Mesh(
      new THREE.BoxGeometry(5.6, 0.07, 0.04),
    ...
    const glow = new THREE.Mesh(
      new THREE.BoxGeometry(5.6, 0.22, 0.04),
      new THREE.MeshBasicMaterial({
        color: "#c8962e",
        transparent: true,
        opacity: 0.0,
    ...
    const leftRoll = new THREE.Mesh(
      new THREE.CylinderGeometry(0.07, 0.07, 0.22, 24),
      new THREE.MeshBasicMaterial({ color: "#5c3a1e", transparent: true, opacity: 0.52 })
    );
    ...
    const sparkleMesh = new THREE.Points(sparkleGeom, sparkleMat);
    ...
    function render(now: number) {
      ...
      (glow.material as THREE.MeshBasicMaterial).opacity = pulse * Math.min(1, fill.scale.x * 5);
